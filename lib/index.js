...
Dovetail.prototype.resolve = function (_middleware) {
  ...
  return _.map(_.keys(actualMiddleware), function (key) {
    ...
    middleware.options.event = middleware.options.event || (middleware.options.event = middleware.options.events || 'default');
    middleware.options.name = middleware.options.name || key;
    middleware.options.app = middleware.options.app || self.app;
    middleware.options.logger = middleware.options.logger || self.logger;
    return self.use(middleware, middleware.options);
  });
};
